...
const ViewStudents = () => {
  ...
  useEffect(() => {
    axios.post(`http://localhost:8000/viewstudents`)
      .then((result) => {
            ...
            <Table style={{ marginTop: "70px" }} sx={{ minWidth: 700 }} aria-label="customized table">
              <TableHead className="viewhead">
                <TableRow>
                  ...
                  <TableCell style={{ textAlign: "center", fontWeight: "600" }}>Course</TableCell>
  ...
  {i.map((row) => (
    <TableRow key={row._id}>
      ...
      <TableCell style={{ textAlign: "center" }}>{row.course}</TableCell>
      <TableCell style={{ textAlign: "center" }}>{row.email}</TableCell>
      {utype === "admin" && (
        <TableCell style={{ textAlign: "center" }}>
          <Link to={`/viewsinglestudent/${row._id}`}>
            <Button
              variant="outlined"
              color="primary"
